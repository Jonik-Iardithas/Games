...
$Global:AppDir = "$env:LOCALAPPDATA\PowerShellTools\Games"
$Global:Archive = "$PSScriptRoot\Games.zip"
$Global:Icons = "$PSScriptRoot\Games - Icons_Custom.zip"
...
$Txt_List = @{
    Form       = "Installer für Games"
    LabelA0    = "Dieser Installer bereitet das Programm Games für die Installation vor."
                                ...
                                If ($clb_Box.GetItemCheckState(0) -eq [System.Windows.Forms.CheckState]::Checked)
                                    {
                                        $ht_Data = @{
                                                    ...
                                                    Arguments = "-ExecutionPolicy Bypass -WindowStyle Hidden -File `"$Global:Path\Games.ps1`""
                                                    WorkingDirectory = $Global:Path
                                                    IconLocation = "$Global:Path\Icons\Games.ico"
                                                    }
